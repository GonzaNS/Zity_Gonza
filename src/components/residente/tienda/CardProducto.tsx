// Sprint 10/11 · HU-TIENDA-05/03 — Tarjeta de producto del catálogo del residente.
// Foto, categoría, nombre y precio (abre el detalle al hacer click) + control de
// carrito (Agregar / − cantidad +). Badge de stock: 'Pocas unidades' (≤5) o
// 'Agotado' (0); la tarjeta agotada se atenúa.
...
export default function CardProducto({ producto, fotoUrl, onClick }: Props) {
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-warm-200 overflow-hidden hover:border-primary-300 hover:shadow-sm transition-all flex flex-col ${
        agotado ? 'opacity-70' : ''
      }`}
    >
      <button
      ...
      >
        ...
        <div className="px-3 pt-3 sm:px-4">
          ...
          <p className="font-display font-semibold text-primary-900 mt-1 sm:text-lg">
            {formatearPrecio(producto.precio)}
          </p>
        </div>
      </button>

      {/* Control de carrito (Sprint 11) — hermano del botón de detalle (no anidado). */}
      <div className="px-3 pb-3 pt-2 sm:px-4 mt-auto">
