...
* **Seguridad (RLS)**: La función SQL usa `SECURITY DEFINER` para permitir consultar las tablas protegidas y emite un chequeo de JWT usando `auth.jwt() -> 'app_metadata' ->> 'rol'` asegurando de manera absoluta que solo el rol `admin` pueda desencadenar esta exportación de datos masiva.
